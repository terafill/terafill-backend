...
run_local_docker:
	docker run -e PORT=8001 -e WORKERS=2 --cpuset-cpus="0-1" --memory=2g --rm --name terafill_backend1 --network host -h 127.0.0.1 -v $(shell pwd):/terafill_backend -d terafill-backend
	docker run -e PORT=8002 -e WORKERS=2 --cpuset-cpus="2-3" --memory=2g --rm --name terafill_backend2 --network host -h 127.0.0.1 -v $(shell pwd):/terafill_backend -d terafill-backend
	docker run -e PORT=8003 -e WORKERS=2 --cpuset-cpus="4-5" --memory=2g --rm --name terafill_backend3 --network host -h 127.0.0.1 -v $(shell pwd):/terafill_backend -d terafill-backend
	# docker run --cpuset-cpus="0-3" --memory=4g --rm --name terafill_backend --network host -p 0.0.0.0:8000:8000 -v $(shell pwd):/terafill_backend -d terafill-backend
...
run_mysql_local:
	docker run --cpuset-cpus="6-7" --memory=4g --rm -p 3306:3306 --name terafill_mysql -d terafill-mysql

run_mysql_cli_local:
	docker run --rm -it --network host mysql mysql -h 0.0.0.0 -u root -p

cleanup:
	docker stop terafill_mysql terafill_backend1 terafill_backend2 terafill_backend3 terafill_backend