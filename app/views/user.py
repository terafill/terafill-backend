...
def read_master_password(db: Session = Depends(get_db), current_user: models.User = Depends(get_current_user)):
...
def update_master_password(master_password: schemas.MasterPasswordUpdate, db: Session = Depends(get_db), current_user: models.User = Depends(get_current_user)):
...
def delete_master_password(db: Session = Depends(get_db), current_user: models.User = Depends(get_current_user)):
    db_master_password = crud.get_master_password(db, user_id=current_user.id)
    if db_master_password is None:
        raise HTTPException(status_code=404, detail="Master Password not found")
...
def delete_user(user_id: int, db: Session = Depends(get_db), current_user: models.User = Depends(get_current_user)):
    ...
    if db_user is None:
        raise HTTPException(status_code=404, detail="User not found")
    response = crud.delete_user(db=db, db_user=db_user)
